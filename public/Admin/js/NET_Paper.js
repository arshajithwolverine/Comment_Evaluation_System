...
async function AddToDatabase(docid) {
  ...
  if (checkempty("Name", Name) || checkempty("Price", Price) || checkempty("Index", Index)) {
    return;
  }
  ...
  for (let index = 0; index < typetopic.length; index++) {
    ...
    if (element == "Note") {
      if (subjectpdf[index].files[0] != undefined) {
        ...
        uploadpdf_wilson(docid, '', true, filelocation, subjectpdf[index].files[0], null, null);
      }
...
async function AddToDatabase_study_material(docid) {
  ...
  if (checkempty("Index", study_index) || checkempty("Main Subject Name", mainsubjectname) || checkempty("Price", price)) {
    return;
  }
...
async function mock_sub(docid) {
  if (docid == null) {
              ...
              <div class="row">
                <div class="col">
                        ...
                        <div class="form-group">
                          ...
                          <select class="form-control" id="select-paper" onchange="ugc_net_paper_select(this,'name')">
                         </select>
                        </div>
    ...
    db.collection("Mock_Test").doc(docid).get().then(async doc => {
                                ...
                                <table class="table align-items-center">
                                    <thead class="thead-light">
                                        <tr>
                                            ...
                                            <th scope="col" class="sort"><span class="btn btn-sm btn-dark" onclick="net_AddNewFeild_mock('${docid}')" >New Field</span></th>
                                            <th></th>
                                        </tr>
                                    </thead>
                                  <tbody id="tableadd" class="list">
      ...
      db.collection("Mock_Test").doc(docid).collection('Tests').orderBy('demo', 'desc').orderBy('index').onSnapshot(snapshot => {
        let html2 = '';
        snapshot.forEach((data) => {
          if (data.data().demo) {
            add = `<tr>
            <td >
             <a href="#" onclick="testview('${docid}','${data.id}')"> <u> ${data.data().name} (Demo)</u></a>
          </td>
...
// two orderby in mock link: https://console.firebase.google.com/v1/r/project/estudo-af035/firestore/indexes?create_composite=Ckpwcm9qZWN0cy9lc3R1ZG8tYWYwMzUvZGF0YWJhc2VzLyhkZWZhdWx0KS9jb2xsZWN0aW9uR3JvdXBzL1Rlc3RzL2luZGV4ZXMvXxABGggKBGRlbW8QAhoJCgVpbmRleBABGgwKCF9fbmFtZV9fEAE
...
async function org_mock_test(docid, mockid) {
  ...
  add = `
          ...
          <!-- Card Header -->
                      <div class="card-header bg-transparent border-0" style="padding-bottom: inherit;">
                              ...
                              <div class="form-group" style="display:flex">
                                        <label for="example-text-input" class="form-control-label" style="margin-right:10px">Demo</label>
                                          <label class="custom-toggle">
                                          <input type="checkbox" id="demo-active">
                                          <span class="custom-toggle-slider rounded-circle"></span>
                                        </label>
                              </div>
                              <div class="form-group">
  ...
  try {
    document.getElementById('demo-active').checked = snap.data().demo;
  } catch (error) {
    console.log("demo-active error skipped");
  }
  try {
...
async function AddMockTest_org(docid, mockid) {
  ...
  progressbar = document.getElementsByName("progressbar");
  mock_active = document.getElementById('mock-active').checked;
  demo_active = document.getElementById('demo-active').checked;
...
function net_AddNewFeild_mock(docid) {
